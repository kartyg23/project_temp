...
class CFG(nn.Module):
    def __init__(self, betaStart, betaEnd,timesteps, UNetConfig, checkpoint = None,num_classes=10,class_emb_size=5):

        super().__init__()
        # The embedding layer will map the class label to a vector of size class_emb_size
        self.device = "cuda" if torch.cuda.is_available() else "cpu"
        self.class_emb = nn.Embedding(num_classes, class_emb_size).to(self.device)
